...
export const SubscriptionScreen = (props: RootStackScreenProps<"Subscription">) => {
    ...
    >
        ...
        >
                        ...
                        [
                            {
                                ...
                                field: (item) => {
                                    ...
                                    return <ProfileButton userId={user.id} size={20}
            ...
            {
                appUser?.settings?.analyst ?
                    <Tab key={"B"} title="Subscribers">
                        <Table
                            rowPressed={(r) => {
                                props.navigation.navigate("Profile", {
                                    userId: r.subscription?.[0].user_id
...
export const SubscriptionSettingsView = (props: { navigation?: NavigationProp<any>, submitRef?: React.MutableRefObject<{ submit: () => Promise<void> }> }) => {
    ...
    return <ElevatedSection title="Subscription" style={{minWidth: '100%'}}>
        ...
        <SwitchField label='Approve New Subscribers?'
                     ...
                     onChange={() => {
                         subscription.update({settings: {approve_new: !subscription.data?.settings.approve_new}})
                     }}
                     viewStyle={{
        ...
        {!props.submitRef?.current &&
            <PrimaryButton disabled={!subscription.hasChanged} onPress={submit}>Apply</PrimaryButton>}
    </ElevatedSection>
}