...
import { CommentsList, CommentsListv2 } from "../components/CommentList";
...
export function ProfileScreen(props: RootStackScreenProps<'Profile'>) {
    ...
    const [user, setUser] = useState<IUserGet>(),
        ...
        [comments, setComments] = useState<AwaitedReturn<typeof Api.User.extensions.getComments>>(),
    ...
    return <View style={[flex]}>
        ...
        {tab === 3 && <KeyboardAvoidingInput
            ...
            onClick={(r: any, s: any, t: any) => {
                if (r.length && loginState?.appUser?.id) {
                    Api.Comment.insert({ user_id: loginState.appUser.id, related_type: 'user', comment: r, related_id: userId })
                    setComments([{
                        id: 0,
                        created_at: String(Date()),
                        comment: r,
                        profile: loginState.appUser,
                        user_id: loginState.appUser.id
                    }, ...(comments || [])])
                    Keyboard.dismiss()
                }
                s('')
