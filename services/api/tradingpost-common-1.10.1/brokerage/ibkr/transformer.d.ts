import { GetSecurityBySymbol, IbkrAccount, IbkrActivity, IbkrPosition, IbkrSecurity, OptionContract, OptionContractWithSymbol, TradingPostBrokerageAccountsTable, TradingPostCurrentHoldingsTableWithMostRecentHolding } from "../interfaces";
...
export default class IbkrTransformer extends BaseTransformer {
    ...
    accounts: (processDate: DateTime, tpUserId: string, accounts: IbkrAccount[]) => Promise<number[]>;
    ...
    _getAccounts: <T extends {
        accountId: string;
    }>(tpUserId: string, ibkrWithAccount: T[]) => Promise<Map<string, TradingPostBrokerageAccountsTable>>;
    _getSecurities: <T extends {
        symbol: string | null;
    }>(ibkrWithSymbols: T[]) => Promise<Map<string, GetSecurityBySymbol>>;
    _getOptions: <T extends {
        symbol: string | null;
        securityDescription: string | null;
    }>(ibkrOptions: T[]) => Promise<Map<string, OptionContractWithSymbol>>;
}
