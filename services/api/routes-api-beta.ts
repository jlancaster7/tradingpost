...
export const createRouterForApi = (versionCode: string) => {
        ...
        router[asGet ? "get" : "post"](path, async (req: Express.Request, res: Express.Response) => {
            try {
                ...
                if (ex instanceof PublicError) {
                    res.status((ex as any).statusCode).json({
                        statusCode: (ex as any).statusCode,
    ...
    const sharedHandler = async (req: Express.Request, routeDetails: (entity: any /* EntityApi<any, any, any, any>*/) => Promise<void>) => {
...
export default createRouterForApi;
