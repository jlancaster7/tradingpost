...
import { PublicError } from '../api/entities/static/EntityApiBase';
...
export class OpenAIClass {
    ...
    getChatResponse = async (model: string, messages: ChatCompletionRequestMessage[], optionalParams?: Omit<Omit<ChatResponseInput, 'model'>, 'messages'>) => {
        let modelInput: ChatResponseInput;

        if (optionalParams) modelInput = { model, messages,...optionalParams }
        else modelInput = { model: model, messages }

        const response = await this.openAI.createChatCompletion(modelInput);
        if (response.status !== 200) {
            throw new PublicError("Call to OpenAI failed. Please try again!", 401);
        }
        return response.data;
